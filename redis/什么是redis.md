...
![布隆过滤器原理.png](https://upload-images.jianshu.io/upload_images/21849446-e3e154a582a485cd.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)

核心是实现一个超大的位数组 和 k 个hash函数。假设位数组的长度为m，哈希函数的个数是k；
具体的操作流程就是：
Q1: 我有了一个 `集合A={x1, y1, x2, y2, ..., xn, yn}` , 很大，可能有40000000个，我要判断一下，`w` 在不在 集合A中，怎么做？
- 设置hash函数的个数：K
- 将超大的位数组进行初始化，将里面每个位都置为 0
- 然后将 `集合A` 中的每个参数，都经过K个hash运算，比如，x1 将得到k个hash值`{a1, a2, a3, ..., ak}`，并将超大位上对应的hash位置1
```
超大的位数组[a1] = 1;
超大的位数组[a2] = 1;
超大的位数组[a3] = 1;
...
```
- 同理处理集合A的其他值，也是一样
- 集合A中所有值都处理完了，我们再来把 `w` 做 k 个hash运算，看看 `超大的位数组[kw1], 超大的位数组[kw2], ..., 超大的位数组[kwk]` 是否有 **0**
...
##### 回到缓存穿透
***扯远了扯远了，我们继续回到缓存穿透***
这里我们说到缓存和数据库里面都没有id，但某人拿着不存在的Id一直请求，很有可能将我们的服务打死，所以，加了一个布隆过滤器之后，就可以先判断这个id是否存在，如果不存在，就不用给他浪费资源了

#### 缓存击穿
和缓存雪崩有一点点像，但又不一样，缓存雪崩是因为大面积的缓存失效，导致大量的请求打到了数据库，数据库被打崩了；
**缓存击穿** 是因为 大量的请求访问一个热点数据，（很多的用户访问一个key，称为热点数据），然后这个热点数据失效了，导致这些请求都打到了数据库上面；
解决办法就是设置热点数据永不过期，，或者加上互斥锁(拿到锁的才能去数据库访问，没拿到的，等几毫秒，看看锁开了没有，开了再去查)
